package dev.aurapvp.samurai.nametag.packet;

import dev.aurapvp.samurai.Samurai;
import dev.aurapvp.samurai.nametag.NameTag;
...
public class ScoreboardTeamPacket {
	...
	public ScoreboardTeamPacket(NameTag tag, Collection<?> players, int paramInt) {
		packet = createPacket();

		setupDefaults(tag.getName(), paramInt);

		try {
			if (paramInt == 0 || paramInt == 2) {
				TEAM_NAME.set(this.packet, tag.getName());
				PARAM_INT.set(this.packet, paramInt);

				PUSH.set(this.packetParams, "never");
				TEAM_COLOR.set(this.packetParams, tag.getChatFormat());
				DISPLAY_NAME.set(this.packetParams, Array.get(CraftChatMessage.fromString(tag.getName()), 0));
				SUFFIX.set(this.packetParams, Array.get(CraftChatMessage.fromString(tag.getSuffix()), 0));

				if (tag.isFriendly() && tag.isFriendlyInvis()) {
					...
					VISIBILITY.set(this.packetParams, tag.isFriendly() ? "never" : "always");
					PREFIX.set(this.packetParams, Array.get(CraftChatMessage.fromString(tag.getPrefix()), 0));
	...
	public ScoreboardTeamPacket(String name, Collection<?> players, int paramInt) {
		packet = createPacket();
	...
	static {
		try {
			...
			playerConnection = typeNMSPlayer.getDeclaredField("c");
