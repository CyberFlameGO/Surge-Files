...
public abstract class CustomEnchant {
    ...
    public CustomEnchant() {
        file = new Config(Samurai.getInstance(), getName(), Samurai.getInstance().getEnchantHandler().getEnchantDirectory());
    ...
    public void apply(Player player, int level) {
        player.setMetadata(META_DATA(), new FixedMetadataValue(Samurai.getInstance(), level));
    }

    public void apply(Player player) {
        player.setMetadata(META_DATA(), new FixedMetadataValue(Samurai.getInstance(), true));
    }

    public void unApply(Player player) {
        player.removeMetadata(META_DATA(), Samurai.getInstance());
    }
    ...
    public boolean hasEnchant(ItemStack stack) {
        return Samurai.getInstance().getEnchantHandler().getCustomEnchants(stack).containsKey(this);
    }
    ...
    public List<String> validateApplicable(List<String> strings) {
        return strings.stream().filter(Samurai.getInstance().getEnchantHandler().getVALID_APPLICABLE()::contains).collect(Collectors.toList());
    }
    ...
    public void registerListener(Listener listener) {
        Samurai.getInstance().getServer().getPluginManager().registerEvents(listener, Samurai.getInstance());
    }
}
