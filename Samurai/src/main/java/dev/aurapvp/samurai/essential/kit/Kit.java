...
public class Kit {
    ...
    public void load() {
        ...
        this.category = config.contains("category") ? Samurai.getInstance().getKitHandler().getCategories().get(config.getString("category").toLowerCase()) : Samurai.getInstance().getKitHandler().getCategories().values().stream().findFirst().get();
    ...
    public void apply(Player player, boolean forced) {
        Optional<KitCooldown> cooldownOpt = Samurai.getInstance().getKitHandler().getCooldown(player.getUniqueId(), this.name);

        if (!player.hasPermission("samurai.kit." + this.name) || cooldownOpt.isPresent()) return;
        if (!forced) Samurai.getInstance().getKitHandler().applyCooldown(player.getUniqueId(), this);
