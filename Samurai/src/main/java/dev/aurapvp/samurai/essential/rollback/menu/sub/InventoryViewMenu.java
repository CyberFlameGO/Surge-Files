package dev.aurapvp.samurai.essential.rollback.menu.sub;
...
public class InventoryViewMenu extends Menu {
    ...
    public List<Button> getButtons(Player player) {
        ...
        i = 9;
        for (int j = 0; j < 9; j++) {
            buttons.add(Button.fromItem(new ItemBuilder(Material.LEGACY_STAINED_GLASS_PANE, 1, (byte) 1).setName(" ").create(), ++i));
        }
    ...
    public class ItemButton extends Button {
        ...
        public ItemStack getItem() {
            ...
            ItemCache itemCache = Samurai.getInstance().getLocatorHandler().fetchCache(stack);
        ...
        public void action(InventoryClickEvent event) throws Exception {
            ...
            if (itemCache.getLocation().getType() == LocationType.BLOCK_INVENTORY) {
                ...
                Samurai.getInstance().getEnderchestHandler().openEnderChest(player, itemCache.getLocation().getHolderUUID());
                ...
                OfflineData offlineCache = Samurai.getInstance().getOfflineHandler().fetchCache(offlinePlayer.getUniqueId());
    ...
    public class RefundButton extends Button {
        ...
        public ItemStack getItem() {
            if (death.isResolved()) {
                if (player.hasPermission("Samurai.command.refund.override")) {
        ...
        public void action(InventoryClickEvent event) throws Exception {
            ...
            SamuraiPlayer victimPlayer = Samurai.getInstance().getPlayerHandler().loadPlayer(death.getVictim(), true);

            victimPlayer.setPlayerDeaths(deaths);
            victimPlayer.updated();
            Samurai.getInstance().getRollbackHandler().updateCache(death.getVictim(), deaths);
        ...
        public class ProofButton extends Button {
            ...
            public void action(InventoryClickEvent event) throws Exception {
                ...
                if (event.getClick() == ClickType.RIGHT) {
                    ...
                    victimPlayer.setPlayerDeaths(deaths);
                    victimPlayer.updated();
                    Samurai.getInstance().getRollbackHandler().updateCache(menu.getDeath().getVictim(), deaths);
                    return;
                }
        ...
        public class AddProofButton extends Button {
            ...
            public void action(InventoryClickEvent event) throws Exception {
                ...
                player.beginConversation(builder.closeableStringPrompt(CC.translate("&aPlease type the proof to add to this players death. Type 'cancel' to stop this process."), (ctx, response) -> {
                            ...
                            SamuraiPlayer victimPlayer = Samurai.getInstance().getPlayerHandler().loadPlayer(menu.getDeath().getVictim(), false);
                            victimPlayer.setPlayerDeaths(menu.getDeaths());
                            victimPlayer.updated();

                            Samurai.getInstance().getRollbackHandler().updateCache(menu.getDeath().getVictim(), menu.getDeaths());
