...
import dev.aurapvp.samurai.essential.kit.menu.KitCategorySelectorMenu;
...
public enum EditorType {
    ...
    RENAME(24, "&aType what you would like the new name of this kit to be. Type 'cancel' to stop this process.", new ItemBuilder(Material.NAME_TAG).setName("&dChange Name &7(Click)").create(), (p, kit, value) -> {
        kit.getFile().getFile().renameTo(new File(Samurai.getInstance().getKitHandler().getKitsDirectory(), value + ".yml"));
        kit.setFile(new Config(Samurai.getInstance(), value, Samurai.getInstance().getKitHandler().getKitsDirectory()));
        kit.setName(value);
    }),
    COOLDOWN(25, "&aType what you would like the new cooldown to be (Ex: 1h, 2h). Type 'cancel' to stop this process.", new ItemBuilder(Material.CLOCK).setName("&6Change Cooldown &7(Click)").create(), (p, kit, value) -> {
        kit.setCooldown(TimeUtils.parseTime(value) * 1000L);
    }),
    GLOWING(26, "&aType true if you want the display item to glow and false if not. Type 'cancel' to stop this process.", new ItemBuilder(Material.ENCHANTING_TABLE).setName("&dGlowing &7(Click)").create(), (p, kit, value) -> {
        try {
    ...
    CHANGE_DISPLAY(30, "&aType what you would like the new display name of this kit to be. Type 'cancel' to stop this process.", new ItemBuilder(Material.OAK_SIGN).setName("&dChange Display &7(Click)").create(), (p, kit, value) -> kit.setDisplayName(value)),
