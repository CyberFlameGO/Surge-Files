...
import dev.aurapvp.samurai.battlepass.challenge.ChallengeListener;
import dev.aurapvp.samurai.battlepass.challenge.category.ChallengeCategory;
import dev.aurapvp.samurai.battlepass.listener.BattlePassListener;
...
//    private final PagedGUISettings rewardsSettings, challengeCategorySettings, challengesSettings;
    ...
    public BattlePassHandler() {
        ...
        this.battlePassYML = new YamlDoc(Samurai.getInstance().getDataFolder(), "battle-pass.yml");
    }
    ...
    private void loadDirectories() {
        this.challengesFolder = new File(Samurai.getInstance().getDataFolder(), "challenges");
        this.tiersFolder = new File(Samurai.getInstance().getDataFolder(), "tiers");
    ...
    private void loadChallenges() {
        for (String key : this.challengesFolder.list()) {
            ...
            this.challenges.add(new Challenge(new Config(Samurai.getInstance(), name, this.challengesFolder)));
        }
    ...
    private void loadDefaultChallenges() {
        ...
        this.challenges.add(new Challenge(new Config(Samurai.getInstance(), "kill_5_zombies", this.challengesFolder)));
        this.challenges.add(new Challenge(new Config(Samurai.getInstance(), "kill_10_zombies", this.challengesFolder)));
        this.challenges.add(new Challenge(new Config(Samurai.getInstance(), "mine_10_dirt", this.challengesFolder)));
        this.challenges.add(new Challenge(new Config(Samurai.getInstance(), "mine_10_grass", this.challengesFolder)));
        this.challenges.add(new Challenge(new Config(Samurai.getInstance(), "place_10_stone", this.challengesFolder)));
    }

    private void loadDefaultTiers() {
        if (!this.tiers.isEmpty()) return;

        this.tiers.put(1, new Tier(new Config(Samurai.getInstance(), "1", this.tiersFolder)));
        this.tiers.put(2, new Tier(new Config(Samurai.getInstance(), "2", this.tiersFolder)));
        this.tiers.put(3, new Tier(new Config(Samurai.getInstance(), "3", this.tiersFolder)));
        this.tiers.put(4, new Tier(new Config(Samurai.getInstance(), "4", this.tiersFolder)));
        this.tiers.put(5, new Tier(new Config(Samurai.getInstance(), "5", this.tiersFolder)));
    }

    private void loadListeners() {
        Bukkit.getPluginManager().registerEvents(new BattlePassListener(), Samurai.getInstance());
        Bukkit.getPluginManager().registerEvents(new ChallengeListener(), Samurai.getInstance());
    }
    ...
    public BattlePass loadBattlePass(UUID uuid, boolean async) {
        SamuraiPlayer player = Samurai.getInstance().getPlayerHandler().loadPlayer(uuid, async);
