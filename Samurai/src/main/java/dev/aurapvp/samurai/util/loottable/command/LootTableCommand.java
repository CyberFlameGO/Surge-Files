package dev.aurapvp.samurai.util.loottable.command;
...
import dev.aurapvp.samurai.util.loottable.editor.menu.LootTablesMenu;
import dev.aurapvp.samurai.util.loottable.menu.LootTablePreviewMenu;
...
public class LootTableCommand extends BaseCommand {
    ...
    public static void reload(CommandSender sender, @Name("loottable") @Optional LootTable crate) {
        if (crate == null) {
            for (LootTable rl : Samurai.getInstance().getLootTableHandler().getLootTables()) {
                rl.reload();
            }
            return;
        }
    ...
    public static void getitems(Player sender, @Name("loottable") LootTable crate) {
        sender.getInventory().setContents(crate.getItems().values()
                .stream()
                .map(item -> {
                    dev.aurapvp.samurai.util.ItemBuilder builder = new ItemBuilder(item.getItem())
