...
public class EditLootTableMenu extends PagedMenu {
    ...
    public List<Button> getGlobalButtons(Player player) {
        ...
        buttons.add(new Button() {
            ...
            public ItemStack getItem() {
                if (massEditing) {
                    return new ItemBuilder(Material.GREEN_BANNER).setName("&aClick to edit all items selected.").create();
                }
                return new ItemBuilder(Material.GREEN_BANNER).setName("&aCreate a new empty item.").create();
            }
    ...
    public class MassEditToggleButton extends Button {
        ...
        @Override
        public ItemStack getItem() {
            if (!massEditing) return new ItemBuilder(Material.RED_WOOL).setName("&fMass Edit&7: &cOff").setLore(Collections.singletonList("&7Click to toggle on")).create();
            return new ItemBuilder(Material.GREEN_WOOL).setName("&fMass Edit&7: &aOn").setLore(Collections.singletonList("&7Click to toggle off")).create();
        }
