...
public class QueueBedrockScoreboard extends HubScoreboard {

    private final Config config;
    private final ScoreboardTitle title;
    private final List<ScoreboardLine> lines = new ArrayList<>();

    public QueueBedrockScoreboard() {
        this.config = new Config(lHub.getInstance(), "bedrock-queue", lHub.getInstance().getScoreboardHandler().getScoreboardDirectory());
    ...
    public boolean qualifies(Player player) {
        ...
        return player.getName().startsWith(".") && queueImpl.inQueue(player);
    }
}
