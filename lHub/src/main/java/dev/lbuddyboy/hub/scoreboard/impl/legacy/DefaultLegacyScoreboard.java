...
public class DefaultLegacyScoreboard extends HubScoreboard {

    private final Config config;
    private final ScoreboardTitle title;
    private final List<ScoreboardLine> lines = new ArrayList<>();

    public DefaultLegacyScoreboard() {
        this.config = new Config(lHub.getInstance(), "legacy-default", lHub.getInstance().getScoreboardHandler().getScoreboardDirectory());
    ...
    public boolean qualifies(Player player) {
        return Via.getAPI().getPlayerVersion(player.getUniqueId()) <= 340;
    }
}
