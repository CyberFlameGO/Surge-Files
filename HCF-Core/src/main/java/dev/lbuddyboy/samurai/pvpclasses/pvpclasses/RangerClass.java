...
public class RangerClass extends PvPClass {
	...
	public void apply(Player player) {
		...
		player.addPotionEffect(new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, PotionEffect.INFINITE_DURATION, 0), true);
	}
	...
	public void onProjectileLaunchEvent(ProjectileLaunchEvent event) {
		if (event.getEntity() instanceof Snowball && event.getEntity().getShooter() instanceof Player) {
			...
			snowball.setMetadata("ShotFromDistance", new FixedMetadataValue(Samurai.getInstance(), snowball.getLocation()));
	...
	public void onEntityDamageByEntityEvent(EntityDamageByEntityEvent event) {
		...
		if (event.getEntity() instanceof Player && snowball.getShooter() instanceof Player) {
			...
			Team shooterTeam = Samurai.getInstance().getTeamHandler().getTeam(shooter);
			...
			}.runTaskLaterAsynchronously(Samurai.getInstance(), 20L * TAG_SECONDS);
		}
	}
