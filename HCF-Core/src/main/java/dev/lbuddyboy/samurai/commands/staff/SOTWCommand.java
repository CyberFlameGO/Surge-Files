...
import dev.lbuddyboy.flash.command.essentials.FlashCommand;
...
public class SOTWCommand extends BaseCommand {
    ...
    public static void unlockserver(CommandSender sender) {
        ...
        Bukkit.dispatchCommand(sender, "flash wellington 2.75");
    ...
    public static void sotwStart(CommandSender sender, @Name("time") String time) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                fixinvis(sender);
/*                if (lastSpoofedUpdated + (60_000 * 30) > System.currentTimeMillis()) return;
                if (lastSpoofedAmount <= 1.35) return;

                lastSpoofedAmount = lastSpoofedUpdated - 0.25;
                lastSpoofedUpdated = System.currentTimeMillis();
                FlashCommand.wellington(Bukkit.getConsoleSender(), lastSpoofedAmount);*/
            }
        }.runTaskTimerAsynchronously(Samurai.getInstance(), 20, 20 * 90);
