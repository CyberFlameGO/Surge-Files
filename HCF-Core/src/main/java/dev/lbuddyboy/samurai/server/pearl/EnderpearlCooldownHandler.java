package dev.lbuddyboy.samurai.server.pearl;

import lombok.Getter;
import dev.lbuddyboy.samurai.Samurai;
import dev.lbuddyboy.samurai.server.event.EnderpearlCooldownAppliedEvent;
...
public class EnderpearlCooldownHandler implements Listener {
	...
	public void onProjectileLaunch(ProjectileLaunchEvent event) {
		...
		if (event.getEntity() instanceof EnderPearl) {
			if (Samurai.getInstance().getMapHandler().isKitMap() && shooter.getWorld().getEnvironment() == World.Environment.THE_END) return;

			// Store the player's enderpearl in-case we need to remove it prematurely
			shooter.setMetadata("LastEnderPearl", new FixedMetadataValue(Samurai.getInstance(), event.getEntity()));

			// Get the default time to apply (in MS)
			long timeToApply = DTRBitmask.THIRTY_SECOND_ENDERPEARL_COOLDOWN.appliesAt(event.getEntity().getLocation()) ? 30_000L : Samurai.getInstance().getMapHandler().getScoreboardTitle().contains("Staging") ? 1_000L : 16_000L;
			...
			Samurai.getInstance().getServer().getPluginManager().callEvent(appliedEvent);
	...
	public void onPlayerTeleport(PlayerTeleportEvent event) {
		...
		if (Samurai.getInstance().getPvPTimerMap().hasTimer(event.getPlayer().getUniqueId()) && ownerTo != null) {
	...
	public static void resetEnderpearlTimer(Player player) {
		...
		if (DTRBitmask.THIRTY_SECOND_ENDERPEARL_COOLDOWN.appliesAt(player.getLocation())) {
			...
			duration = (Samurai.getInstance().getMapHandler().getScoreboardTitle().contains("Staging") ? 1_000L : 16_000L);
		}
