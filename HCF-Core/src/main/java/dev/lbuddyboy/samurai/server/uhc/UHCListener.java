package dev.lbuddyboy.samurai.server.uhc;
...
public class UHCListener implements Listener {
    ...
    private void updateToAll(Player player) {
        ...
        try {
            PacketPlayOutScoreboardScore packet = new PacketPlayOutScoreboardScore(null);
            ...
            for (Player onlinePlayer : Bukkit.getOnlinePlayers()) {
                ...
                ((CraftPlayer) onlinePlayer).getHandle().b.a(packet);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void onPrepare(PrepareItemCraftEvent event) {
        if (isOldCrappleRecipe(event.getRecipe())) {
            ItemStack result = ItemBuilder.of(Material.LEGACY_WOOL).data(DyeColor.RED.getWoolData()).name("&cThis recipe is disabled.").addToLore("&eUse gold nuggets instead of ingots.").build();
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        Bukkit.getScheduler().runTaskLater(Samurai.getInstance(), () -> init(event.getPlayer()), 1L);
    }
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        Bukkit.getScheduler().runTaskLater(Samurai.getInstance(), () -> deathTime.remove(event.getEntity().getUniqueId()), 20L);
    }

    @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
    public void onLeafDecay(LeavesDecayEvent event) {
        if (event.getBlock().getType() == Material.LEGACY_LEAVES) {
            if ((Math.random() * 100) <= 3) {
                event.getBlock().getWorld().dropItemNaturally(event.getBlock().getLocation(), new ItemStack(Material.APPLE));
            }
        } else if (event.getBlock().getType() == Material.LEGACY_LEAVES_2) {
            if ((Math.random() * 100) <= 3) {
                event.getBlock().getWorld().dropItemNaturally(event.getBlock().getLocation(), new ItemStack(Material.APPLE));
            }
        }
    ...
    private static double getHearts(Player player) {
        return getHearts0(player.getHealth() + ((CraftPlayer) player).getHandle().getBukkitEntity().getAbsorptionAmount());
    }

    private static int getHealth(Player player) {
        return (int) Math.ceil(player.getHealth() + ((CraftPlayer) player).getHandle().getBukkitEntity().getAbsorptionAmount());
    }
