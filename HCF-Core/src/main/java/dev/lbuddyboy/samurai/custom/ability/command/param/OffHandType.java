...
public final class OffHandType implements ContextResolver<OffHand, BukkitCommandExecutionContext> {

	@Override
	public OffHand getContext(BukkitCommandExecutionContext c) throws InvalidCommandArgument {
		AbilityItemHandler handler = Samurai.getInstance().getAbilityItemHandler();

		AbilityItem partnerPackageByName = handler.getOffHandByName(c.popFirstArg());
		if (partnerPackageByName == null) {
			...
			c.getSender().sendMessage(CC.RED + "No off hand with that name found!");
			c.getSender().sendMessage(CC.RED + "Available: " + CC.YELLOW + available);
		}

		return (OffHand) partnerPackageByName;
	}
}
