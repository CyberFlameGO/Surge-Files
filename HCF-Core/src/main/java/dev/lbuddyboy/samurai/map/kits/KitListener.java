...
public class KitListener implements Listener {
    ...
    @Getter private static List<Entity> ENTITIES = new ArrayList<>();
    ...
    public void onExplode(BlockFormEvent event) {
        ...
        Samurai.getInstance().getMapHandler().getKitManager().getBrokenBlocks().put(event.getNewState().getLocation(), event.getBlock().getBlockData());
    }
    ...
    @EventHandler
    public void onBreak(EntitySpawnEvent event) {
        if (!Samurai.getInstance().getMapHandler().isKitMap()) return;
        if (event.getEntity().getWorld().getEnvironment() != World.Environment.THE_END) return;

        Samurai.getInstance().getMapHandler().getKitManager().getEntities().add(event.getEntity());
    }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (!DTRBitmask.SAFE_ZONE.appliesAt(event.getClickedBlock().getLocation())) return;
