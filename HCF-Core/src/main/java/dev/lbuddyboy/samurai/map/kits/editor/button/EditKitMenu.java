...
public final class EditKitMenu extends Menu {
    ...
    public Map<Integer, Button> getButtons(Player player) {
        ...
        // The vertical row
        for (int i = 0; i <= 5; i++) {
            buttons.put(getSlot(1, i), Button.placeholder(Material.LEGACY_STAINED_GLASS_PANE, (byte) 7));
        }

        // The horizontal row
        for (int i = 0; i <= 8; i++) {
            buttons.put(getSlot(i, 1), Button.placeholder(Material.LEGACY_STAINED_GLASS_PANE, (byte) 7));
        }

        // Fill up the 3 empty slots up top. they can be used to dupe items
        buttons.put(getSlot(3, 0), Button.placeholder(Material.LEGACY_STAINED_GLASS_PANE, (byte) 7));
        buttons.put(getSlot(4, 0), Button.placeholder(Material.LEGACY_STAINED_GLASS_PANE, (byte) 7));
        buttons.put(getSlot(5, 0), Button.placeholder(Material.LEGACY_STAINED_GLASS_PANE, (byte) 7));
