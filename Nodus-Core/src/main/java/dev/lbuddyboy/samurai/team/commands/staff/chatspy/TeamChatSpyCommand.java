package dev.lbuddyboy.samurai.team.commands.staff.chatspy;
...
public class TeamChatSpyCommand extends BaseCommand {
    ...
    public static void teamChatSpyAdd(Player sender, @Name("team") Team team) {
        if (Samurai.getInstance().getChatSpyMap().getChatSpy(sender.getUniqueId()).contains(team.getUniqueId())) {
            sender.sendMessage(ChatColor.RED + "You are already spying on " + team.getName() + ".");
            return;
        }

        List<ObjectId> teams = new ArrayList<>(Samurai.getInstance().getChatSpyMap().getChatSpy(sender.getUniqueId()));

        teams.add(team.getUniqueId());

        Samurai.getInstance().getChatSpyMap().setChatSpy(sender.getUniqueId(), teams);
    ...
    public static void teamChatSpyClear(Player sender) {
        Samurai.getInstance().getChatSpyMap().setChatSpy(sender.getUniqueId(), new ArrayList<ObjectId>());
    ...
    public static void teamChatSpyList(Player sender) {
        ...
        for (ObjectId team : Samurai.getInstance().getChatSpyMap().getChatSpy(sender.getUniqueId())) {
            Team teamObj = Samurai.getInstance().getTeamHandler().getTeam(team);
