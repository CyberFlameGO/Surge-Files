...
import dev.lbuddyboy.samurai.team.claims.Claim.CuboidDirection;
...
public class VisualClaim implements Listener {
	...
	public static final Material[] MAP_MATERIALS = {Material.DIAMOND_BLOCK,
			Material.GOLD_BLOCK, Material.OAK_LOG, Material.BRICK, Material.OAK_WOOD,
			Material.REDSTONE_BLOCK, Material.LAPIS_BLOCK, Material.CHEST,
			Material.MELON, Material.STONE, Material.COBBLESTONE,
		...
		switch (type) {
			...
			case MAP:
				...
				for (Map.Entry<Claim, Team> regionData : LandBoard.getInstance().getRegionData(player.getLocation(), MAP_RADIUS, 256, MAP_RADIUS)) {
					...
					drawClaim(player.getLocation().getBlockY(), regionData.getKey(), mat);
			...
			case SURFACE_MAP:
				for (Map.Entry<Claim, Team> regionData : LandBoard.getInstance().getRegionData(player.getLocation(), MAP_RADIUS, 256, MAP_RADIUS)) {
					...
					World claimWorld = Samurai.getInstance().getServer().getWorld(claim.getWorld());
					...
					for (Coordinate coordinate : claim) {
						...
						while (!block.getType().isSolid() || block.getType() == Material.LEGACY_LEAVES || block.getType() == Material.LEGACY_LOG || block.getType() == Material.LEGACY_LOG_2) {
						...
						sendBlockChange(player, block.getLocation(), Material.LEGACY_WOOL, carpetColor.getWoolData());
						sendBlockChange(player, block.getRelative(BlockFace.UP).getLocation(), Material.LEGACY_CARPET, carpetColor.getWoolData());
				...
				break;
			case SUBCLAIM_MAP:
				Team senderTeam = Samurai.getInstance().getTeamHandler().getTeam(player);
	...
	public boolean containsOtherClaim(Claim claim) {
		...
		for (int x = minPoint.getBlockX(); x <= maxPoint.getBlockX(); x++) {
			for (int z = minPoint.getBlockZ(); z <= maxPoint.getBlockZ(); z++) {
				Location at = new Location(Samurai.getInstance().getServer().getWorld(claim.getWorld()), x, 80, z);
	...
	public Set<Claim> getTouchingClaims(Claim claim) {
		...
		for (Coordinate coordinate : claim.outset(CuboidDirection.Horizontal, 1)) {
			Location loc = new Location(Samurai.getInstance().getServer().getWorld(claim.getWorld()), coordinate.getX(), 80, coordinate.getZ());
	...
	public void setLoc(int locationId, final Location clicked) {
		...
		if (type == VisualClaimType.CREATE) {
			if (!bypass && !Samurai.getInstance().getServerHandler().isUnclaimed(clicked)) {
			...
			Samurai.getInstance().getServer().getScheduler().runTaskLater(Samurai.getInstance(), () -> erectPillar(player.getLocation().getBlockY(), clicked, Material.EMERALD_BLOCK), 1L);
			...
			if (locationId == 1) {
				...
				drawClaim(player.getLocation().getBlockY(), resizing, Material.LAPIS_BLOCK);
			} else if (locationId == 2) {
				if (resizing == null) {
				...
				new BukkitRunnable() {
					...
					public void run() {
						clearAllBlocks();
						drawClaim(player.getLocation().getBlockY(), resizing, Material.LAPIS_BLOCK);
						drawClaim(player.getLocation().getBlockY(), claimClone, Material.EMERALD_BLOCK);
					}
	...
	public void purchaseClaim() {
		...
		if (corner1 != null && corner2 != null) {
			...
			claim.setName(playerTeam.getName() + "_" + (100 + Samurai.RANDOM.nextInt(800)));
			claim.setY1(player.getWorld().getMinHeight());
			claim.setY2(player.getWorld().getMaxHeight());
			...
			if (!bypass) {
				...
				Samurai.getInstance().getLogger().info("Economy Logger: Withdrawing " + price + " from " + playerTeam.getBalance() + "'s account: Claimed land");
			...
			String pointA = minLoc.getBlockX() + ", " + minLoc.getBlockY() + ", " + minLoc.getBlockZ();
			String pointB = maxLoc.getBlockX() + ", " + maxLoc.getBlockY() + ", " + maxLoc.getBlockZ();
			playerTeam.createLog(player.getUniqueId(), "CLAIM LAND", "1st: " + pointA + " | 2nd: " + pointB);
	...
	private void drawClaim(int playerY, Claim claim, Material material) {
		for (Location loc : claim.getCornerLocations()) {
			erectPillar(playerY, loc, material);
		}
	}
	...
	private void erectPillar(int playerY, Location loc, Material mat) {
		Location set = loc.clone();

		for (int y = 0; y < 20; y++) {
			set.setY(playerY + y);

			if (set.getBlock().getType() == Material.AIR || set.getBlock().getType().isTransparent() || set.getBlock().getType() == Material.WATER || set.getBlock().getType() == Material.WATER) {
				if (y % 5 == 0) {
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		if (event.getPlayer() == player && player.getItemInHand() != null) {
			...
			} else if (player.getItemInHand().getType() == TeamCommands.SELECTION_WAND.getType() && type == VisualClaimType.RESIZE) {
				switch (event.getAction()) {
					case RIGHT_CLICK_BLOCK:
						setLoc(2, event.getClickedBlock().getLocation());
						break;
