...
import dev.lbuddyboy.samurai.server.event.BackstabKillEvent;
...
public class RogueClass extends PvPClass {
    ...
    public void onEntityArrowHit(EntityDamageByEntityEvent event) {
        ...
        if (event.getDamager() instanceof Player && event.getEntity() instanceof Player) {
            ...
            if (damager.getItemInHand() != null && damager.getItemInHand().getType() == Material.GOLDEN_SWORD && PvPClassHandler.hasKitOn(damager, this)) {
                if (backstabCooldown.containsKey(damager.getName()) && backstabCooldown.get(damager.getName()) > System.currentTimeMillis()) {
                    return;
                }

                if (KitDisabler.getDisabled().onCooldown(damager.getUniqueId())) {
                    damager.sendMessage(CC.translate("&c[Kit Disabler] Your kit ability is disabled for " + KitDisabler.getDisabled().getRemaining(damager) + "."));
                    return;
                }
    ...
    public class BackstabDamage extends PlayerDamage {
        ...
        public String getDeathMessage() {
            ...
            return (ChatColor.RED + getDamaged() + ChatColor.DARK_RED + "[" + Samurai.getInstance().getMapHandler().getStatsHandler().getStats(UUIDUtils.uuid(getDamaged())).getKills() + "]" + ChatColor.YELLOW + " was backstabbed by " + ChatColor.RED + getDamager() + ChatColor.DARK_RED + "[" + Samurai.getInstance().getMapHandler().getStatsHandler().getStats(UUIDUtils.uuid(getDamager())).getKills() + "]" + ChatColor.YELLOW + ".");
        }
