...
public class ArcherClass extends PvPClass {
    ...
    public void onEntityArrowHit(EntityDamageByEntityEvent event) {
        if (event.getEntity() instanceof Player && event.getDamager() instanceof Arrow) {
            ...
            if (arrow.hasMetadata("CrossBow")) return;
...
//                victim.addPotionEffect(new PotionEffect(PotionEffectType.GLOWING, 20 * MARK_SECONDS, 2));
            ...
            if (Samurai.getInstance().getBattlePassHandler() != null) {
                Samurai.getInstance().getBattlePassHandler().useProgress(shooter.getUniqueId(), progress -> {
                    progress.setArcherTags(progress.getArcherTags() + 1);
                    progress.requiresSave();

                    Samurai.getInstance().getBattlePassHandler().checkCompletionsAsync(shooter);
                });
            }
        }
    }
    ...
    public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
        if (event.getEntity() instanceof Player) {
            ...
            if (isMarked(player)) {
                ...
                event.setDamage(event.getDamage() * FoxtrotConfiguration.HITS_ARCHER_DAMAGE_MULTIPLIER.getDouble());
            }
        }
    }
    ...
    public void onEntityShootBow(EntityShootBowEvent event) {
        event.getProjectile().setMetadata("Pullback", new FixedMetadataValue(Samurai.getInstance(), event.getForce()));
    }
