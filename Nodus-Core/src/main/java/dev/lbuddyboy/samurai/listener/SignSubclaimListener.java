...
public class SignSubclaimListener implements Listener {
    ...
    public void onBlockBreakChest(BlockBreakEvent event) {
        if (!(event.getBlock().getState() instanceof Chest) || Samurai.getInstance().getServerHandler().isUnclaimedOrRaidable(event.getBlock().getLocation()) || Samurai.getInstance().getServerHandler().isAdminOverride(event.getPlayer())) {
            return;
        }
    ...
    public void onBlockBreakSign(BlockBreakEvent event) {
        if (!(event.getBlock().getState() instanceof Sign) || Samurai.getInstance().getServerHandler().isUnclaimedOrRaidable(event.getBlock().getLocation()) || Samurai.getInstance().getServerHandler().isAdminOverride(event.getPlayer())) {
            return;
        }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        if (event.getAction() != Action.RIGHT_CLICK_BLOCK || !(event.getClickedBlock().getState() instanceof Chest) || Samurai.getInstance().getServerHandler().isUnclaimedOrRaidable(event.getClickedBlock().getLocation())
                || Samurai.getInstance().getServerHandler().isAdminOverride(event.getPlayer())) {
            return;
        }
    ...
    public static void subclaimSigns0(Set<Sign> signs, Block check) {
        for (BlockFace blockFace : OUTSIDE_FACES) {
            ...
            if (relBlock.getType() == Material.LEGACY_WALL_SIGN || relBlock.getType() == Material.LEGACY_SIGN) {
