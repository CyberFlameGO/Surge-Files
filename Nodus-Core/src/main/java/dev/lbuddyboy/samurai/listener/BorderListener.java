...
public class BorderListener implements Listener {
	...
	public static int BORDER_SIZE_NETHER = 1000;
	...
	public void onBlockPlace(BlockPlaceEvent event) {
		...
		if (event.getBlock().getWorld().getEnvironment() == World.Environment.NETHER) {
			if (Math.abs(event.getBlock().getX()) > BORDER_SIZE_NETHER || Math.abs(event.getBlock().getZ()) > BORDER_SIZE_NETHER) {
				event.setCancelled(true);
			}
		}
	}
	...
	public void onPlayerPortal(PlayerPortalEvent event) {
		...
		if (event.getTo().getWorld().getEnvironment() == World.Environment.NETHER) {
			if (Math.abs(event.getTo().getBlockX()) > BORDER_SIZE_NETHER || Math.abs(event.getTo().getBlockZ()) > BORDER_SIZE_NETHER) {
				Location newLocation = event.getTo().clone();

				while (Math.abs(newLocation.getX()) > BORDER_SIZE_NETHER) {
					newLocation.setX(newLocation.getX() - (newLocation.getX() > 0 ? 1 : -1));
				}

				while (Math.abs(newLocation.getZ()) > BORDER_SIZE_NETHER) {
					newLocation.setZ(newLocation.getZ() - (newLocation.getZ() > 0 ? 1 : -1));
				}
	...
	public void onPlayerMove(PlayerMoveEvent event) {
		...
		if (from.getBlockX() != to.getBlockX() || from.getBlockZ() != to.getBlockZ() || from.getBlockY() != to.getBlockY()) {
			if (Math.abs(event.getTo().getBlockX()) > BORDER_SIZE || Math.abs(event.getTo().getBlockZ()) > BORDER_SIZE) {
				...
				if (tries >= 99) {
					Samurai.getInstance().getLogger().severe("The server would have crashed while doing border checks! New X: " + newLocation.getX() + ", Old X: " + event.getTo().getBlockX());
					return;
				}
				...
				if (tries >= 99) {
					Samurai.getInstance().getLogger().severe("The server would have crashed while doing border checks! New Z: " + newLocation.getZ() + ", Old Z: " + event.getTo().getBlockZ());
					return;
				}
			...
			if (event.getTo().getWorld().getEnvironment() == World.Environment.NETHER) {
				if (Math.abs(event.getTo().getBlockX()) > BORDER_SIZE_NETHER || Math.abs(event.getTo().getBlockZ()) > BORDER_SIZE_NETHER) {
					...
					while (Math.abs(newLocation.getX()) > BORDER_SIZE_NETHER && tries++ < 100) {
						newLocation.setX(newLocation.getX() - (newLocation.getX() > 0 ? 1 : -1));
					}
					...
					while (Math.abs(newLocation.getZ()) > BORDER_SIZE_NETHER && tries++ < 100) {
						newLocation.setZ(newLocation.getZ() - (newLocation.getZ() > 0 ? 1 : -1));
					}
