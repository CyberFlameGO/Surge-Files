...
public class BlockConvenienceListener implements Listener {
    ...
    public void onBlockDropItemsEvent(BlockDropItemEvent event) {
...
//        }
        ...
        boolean pickupCobble = Samurai.getInstance().getCobblePickupMap().isCobblePickup(event.getPlayer().getUniqueId());
        ...
        if (!remainingItems.isEmpty()) {
            for (ItemStack item : remainingItems) {
                if (SOTW_IGNORED_BLOCKS.contains(item.getType()) && SOTWCommand.isSOTWTimer()) {
                    continue;
                }
