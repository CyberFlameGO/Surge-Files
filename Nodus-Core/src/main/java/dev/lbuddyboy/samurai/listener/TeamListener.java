...
import dev.lbuddyboy.samurai.team.event.PlayerBuildInOthersClaimEvent;
...
public class TeamListener implements Listener {
    ...
    @EventHandler
    public void onCaptured(EventCapturedEvent event) {
        ...
        team.createLog(event.getPlayer().getUniqueId(), "KOTH CAPTURED", event.getEvent().getDisplayName() + " was captured (" + Samurai.getInstance().getTopHandler().getTotalPoints(team) + ")");
    }
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        if (Samurai.getInstance().getServerHandler().isAdminOverride(event.getPlayer()) || Samurai.getInstance().getServerHandler().isUnclaimedOrRaidable(event.getBlock().getLocation())) {
            return;
        }
...
/*                new BukkitRunnable() {
                ...
                if (event.getBlock().getWorld().getEnvironment() == World.Environment.THE_END && Samurai.getInstance().getMapHandler().isKitMap()) return;
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        ...
        if (event.getBlock().getWorld().getEnvironment() != World.Environment.NETHER && team.hasDTRBitmask(DTRBitmask.ROAD) && event.getBlock().getY() <= 30) {
    ...
    public void onBlockPistonRetract(BlockPistonRetractEvent event) {
        ...
        for (Block block : event.getBlocks()) {
            Team targetTeam = LandBoard.getInstance().getTeam(block.getLocation());
    ...
    public void onHangingPlace(HangingPlaceEvent event) {
        if (Samurai.getInstance().getServerHandler().isAdminOverride(event.getPlayer()) || Samurai.getInstance().getServerHandler().isUnclaimedOrRaidable(event.getEntity().getLocation())) {
            return;
        }
    ...
    public void onHangingBreakByEntity(HangingBreakByEntityEvent event) {
        ...
        if (Samurai.getInstance().getServerHandler().isUnclaimedOrRaidable(event.getEntity().getLocation())) {
            return;
        }
    ...
    public void onPlayerInteractEntityEvent(PlayerInteractEntityEvent event) {
        ...
        if (Samurai.getInstance().getServerHandler().isUnclaimedOrRaidable(event.getRightClicked().getLocation())) {
            return;
        }
    ...
    public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
        ...
        if (damager == null || Samurai.getInstance().getServerHandler().isAdminOverride(damager) || Samurai.getInstance().getServerHandler().isUnclaimedOrRaidable(event.getEntity().getLocation())) {
            return;
        }
    ...
    public void onEntityDamageByEntity2(EntityDamageByEntityEvent event) {
        ...
        if (damager != null) {
            Team team = Samurai.getInstance().getTeamHandler().getTeam(damager);
            Player victim = (Player) event.getEntity();

            if (team != null && event.getCause() != EntityDamageEvent.DamageCause.FALL) {
                if (team.isMember(victim.getUniqueId())) {
                    if (Samurai.getInstance().getServerHandler().isAllowBoosting()) {
    ...
    public void onEntityHorseDamage(EntityDamageByEntityEvent event) {
        ...
        if (damager != null && victim.isTamed()) {
            Team damagerTeam = Samurai.getInstance().getTeamHandler().getTeam(damager);
            ...
            if(!damager.getUniqueId().equals(horseOwner) && damagerTeam != null && damagerTeam.isMember(horseOwner)) {
                event.setCancelled(true);
                damager.sendMessage(ChatColor.YELLOW + "This horse belongs to " + ChatColor.DARK_GREEN + FrozenUUIDCache.name(horseOwner) + ChatColor.YELLOW + " who is in your team.");
            }
        }
    }
    ...
    public void onBucketEmpty(PlayerBucketEmptyEvent event) {
        ...
        if (Samurai.getInstance().getServerHandler().isAdminOverride(event.getPlayer()) || Samurai.getInstance().getServerHandler().isUnclaimedOrRaidable(checkLocation)) {
            return;
        }
        ...
        boolean canPlace = owner.hasDTRBitmask(DTRBitmask.KOTH) && Samurai.getInstance().getServerHandler().isWaterPlacementInClaimsAllowed();
