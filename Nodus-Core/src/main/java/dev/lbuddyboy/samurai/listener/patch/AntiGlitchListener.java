...
import dev.lbuddyboy.samurai.util.MaterialUtils;
import org.bukkit.*;
...
public class AntiGlitchListener implements Listener {
    ...
    public void onSpawn(VehicleCreateEvent event) {
        if (event.getVehicle().getType().name().contains("MINECART")) {
    ...
    public void onBook(PlayerInteractEvent event) {
        ...
        event.setCancelled(true);
        event.getPlayer().sendMessage(CC.translate("&cYou cannot use this item."));
    ...
    public void onBoatMove(VehicleMoveEvent event) {
        ...
        if (block.getType() == Material.LEGACY_FENCE_GATE) {
    ...
    public void onPlayerBoatMove(PlayerMoveEvent event) {
        ...
        if (player.getVehicle() != null && player.getVehicle() instanceof Boat) {
            if (to.getBlock().getType() == Material.LEGACY_FENCE_GATE) {
                event.setCancelled(true);
    ...
    public void denyDismountClipping(VehicleExitEvent event) {
        ...
        Bukkit.getScheduler().runTask(Samurai.getInstance(), () -> player.teleport(finalLocation));
    }
    ...
    public void denyDismountClipping(CreatureSpawnEvent event) {
        ...
        if (Samurai.getInstance().getServerHandler().isHardcore()) {
            return;
        }
