...
public class FoundDiamondsListener implements Listener {
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        if (event.getBlock().getType() == Material.DIAMOND_ORE) {
            event.getBlock().setMetadata("DiamondPlaced", new FixedMetadataValue(Samurai.getInstance(), true));
        }
    }
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        if (event.getBlock().getType() == Material.DIAMOND_ORE && !event.getBlock().hasMetadata("DiamondPlaced")) {
            ...
            for (Player player : Samurai.getInstance().getServer().getOnlinePlayers()) {
                if (Samurai.getInstance().getToggleFoundDiamondsMap().isFoundDiamondToggled(player.getUniqueId())) {
    ...
    public int countRelative(Block block) {
        ...
        block.setMetadata("DiamondPlaced", new FixedMetadataValue(Samurai.getInstance(), true));
        ...
        for (BlockFace checkFace : CHECK_FACES) {
            ...
            if (relative.getType() == Material.DIAMOND_ORE && !relative.hasMetadata("DiamondPlaced")) {
                relative.setMetadata("DiamondPlaced", new FixedMetadataValue(Samurai.getInstance(), true));
