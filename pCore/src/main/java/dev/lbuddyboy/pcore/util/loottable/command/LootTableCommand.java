package dev.lbuddyboy.pcore.util.loottable.command;
...
import dev.lbuddyboy.pcore.util.loottable.editor.menu.LootTablesMenu;
import dev.lbuddyboy.pcore.util.loottable.menu.LootTablePreviewMenu;
...
public class LootTableCommand extends BaseCommand {
    ...
    public static void reload(CommandSender sender, @Name("loottable") @Optional LootTable crate) {
        if (crate == null) {
            for (LootTable rl : pCore.getInstance().getLootTableHandler().getLootTables()) {
                rl.reload();
            }
            return;
        }
    ...
    public static void preview(Player sender, @Name("loottable") LootTable crate) {
        new LootTablePreviewMenu(crate, null).openMenu(sender);
    }
    ...
    public static void createItem(Player sender, @Name("loottable") LootTable lootTable, @Name("id") String id, @Name("useHand") @Optional Boolean useHand) {
        ...
        if (useHand)
            stack = sender.getInventory().getItemInHand();
    ...
    public static void setitem(Player sender, @Name("loottable") LootTable lootTable, @Name("config-id") LootTableItem item) {
        ...
        item.setItem(sender.getInventory().getItemInHand());
        item.setAmount(sender.getInventory().getItemInHand().getAmount());
