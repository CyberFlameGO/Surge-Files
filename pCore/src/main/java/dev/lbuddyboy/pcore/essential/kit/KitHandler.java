...
import dev.lbuddyboy.pcore.essential.kit.command.KitCommand;
...
public class KitHandler implements IModule {
    ...
    private void loadDirectories() {
        this.kitsConfig = new Config(pCore.getInstance(), "kits");
        this.kitsDirectory = new File(pCore.getInstance().getDataFolder(), "kits");
    ...
    public void loadKits() {
        for (String s : this.categoriesDirectory.list()) {
            ...
            this.categories.put(name.toLowerCase(), new KitCategory(new Config(pCore.getInstance(), name, this.categoriesDirectory)));
        }
        ...
        for (String s : this.kitsDirectory.list()) {
            ...
            this.kits.put(name, new Kit(new Config(pCore.getInstance(), name, this.kitsDirectory)));
        }
    ...
    private void loadDefaults() {
        ...
        Tasks.delayed(() -> this.kits.put("Starter", new Kit(new Config(pCore.getInstance(), "Starter", this.kitsDirectory))));
    }

    private void loadCommands() {
        pCore.getInstance().getCommandManager().getCommandCompletions().registerCompletion("kits", s -> kits.values().stream().map(Kit::getName).collect(Collectors.toList()));
        pCore.getInstance().getCommandManager().registerCommand(new KitCommand());
    }
