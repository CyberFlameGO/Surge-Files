...
public class EggTypesCompletion implements CommandCompletions.CommandCompletionHandler<BukkitCommandCompletionContext> {
    ...
    public Collection<String> getCompletions(BukkitCommandCompletionContext context) throws InvalidCommandArgument {
        ...
        if (input.isEmpty()) {
            completions.addAll(pCore.getInstance().getPetHandler().getEggImpls().stream().map(EggImpl::getName).collect(Collectors.toList()));
        } else {
            for (String s : pCore.getInstance().getPetHandler().getEggImpls().stream().map(EggImpl::getName).collect(Collectors.toList())) {
