...
public class RaritiesCompletion implements CommandCompletions.CommandCompletionHandler<BukkitCommandCompletionContext> {
    ...
    public Collection<String> getCompletions(BukkitCommandCompletionContext context) throws InvalidCommandArgument {
        ...
        if (input.isEmpty()) {
            completions.addAll(pCore.getInstance().getPetHandler().getRarities().values().stream().map(PetRarity::getName).collect(Collectors.toList()));
        } else {
            for (String s : pCore.getInstance().getPetHandler().getRarities().values().stream().map(PetRarity::getName).collect(Collectors.toList())) {
