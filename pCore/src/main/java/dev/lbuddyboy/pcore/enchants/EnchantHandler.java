...
import dev.lbuddyboy.pcore.enchants.command.ArmorSetsCommand;
import dev.lbuddyboy.pcore.enchants.command.completion.ArmorSetCompletion;
import dev.lbuddyboy.pcore.enchants.command.context.ArmorSetContext;
import dev.lbuddyboy.pcore.enchants.lore.CustomLoreComparator;
...
import dev.lbuddyboy.pcore.enchants.set.impl.GladiatorArmorSet;
import dev.lbuddyboy.pcore.enchants.thread.EnchantUpdateThread;
import dev.lbuddyboy.pcore.pCore;
import dev.lbuddyboy.pcore.enchants.command.EnchantItemsCommand;
import dev.lbuddyboy.pcore.enchants.command.EnchanterCommand;
import dev.lbuddyboy.pcore.enchants.command.EnchantsCommand;
import dev.lbuddyboy.pcore.enchants.impl.*;
import dev.lbuddyboy.pcore.enchants.listener.CustomEnchantListener;
import dev.lbuddyboy.pcore.enchants.listener.EnchantBookListener;
import dev.lbuddyboy.pcore.enchants.rarity.Rarity;
import dev.lbuddyboy.pcore.util.*;
import dev.lbuddyboy.pcore.util.event.ArmorListener;
...
public class EnchantHandler implements IModule {
    ...
    public EnchantHandler() {
        this.configFile = new Config(pCore.getInstance(), "enchants");
    ...
    public void load(pCore plugin) {
        ...
        Bukkit.getScheduler().runTaskTimer(pCore.getInstance(), () -> {
    ...
    private void loadListeners() {
        Bukkit.getPluginManager().registerEvents(new ArmorListener(Collections.emptyList()), pCore.getInstance());
        Bukkit.getPluginManager().registerEvents(new CustomEnchantListener(), pCore.getInstance());
        Bukkit.getPluginManager().registerEvents(new EnchantBookListener(), pCore.getInstance());
    }

    private void loadCommands() {
        pCore.getInstance().getCommandManager().getCommandCompletions().registerCompletion("rarities", s -> rarities.stream().map(Rarity::getName).collect(Collectors.toList()));
        pCore.getInstance().getCommandManager().getCommandCompletions().registerCompletion("armorsets", new ArmorSetCompletion());
        pCore.getInstance().getCommandManager().getCommandContexts().registerContext(ArmorSet.class, new ArmorSetContext());
        pCore.getInstance().getCommandManager().registerCommand(new ArmorSetsCommand());
        pCore.getInstance().getCommandManager().registerCommand(new EnchanterCommand());
        pCore.getInstance().getCommandManager().registerCommand(new EnchantItemsCommand());
        pCore.getInstance().getCommandManager().registerCommand(new EnchantsCommand());
    }

    private void loadDirectories() {
        this.enchantDirectory = new File(pCore.getInstance().getDataFolder(), "enchants");
