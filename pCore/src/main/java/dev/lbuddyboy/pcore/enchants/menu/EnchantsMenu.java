...
public class EnchantsMenu extends Menu {
    ...
    public List<Button> getButtons(Player player) {
        ...
        int i = 1;
        for (String gear : pCore.getInstance().getEnchantHandler().getGEAR()) {
            Material gearType = Material.getMaterial("LEATHER_" + gear);

            buttons.add(new GearButton(gear, gearType, i));
            i++;
        }
        for (String gear : pCore.getInstance().getEnchantHandler().getNON_GEAR()) {
            Material gearType = Material.getMaterial(gear);

            buttons.add(new GearButton(gear, gearType, i));
            i++;
        }
        for (String gear : pCore.getInstance().getEnchantHandler().getTOOLS()) {
