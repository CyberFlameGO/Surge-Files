package dev.lbuddyboy.pcore.events.koth;

import dev.lbuddyboy.pcore.events.Event;
import dev.lbuddyboy.pcore.events.EventType;
...
public class KoTH extends BukkitRunnable implements Event {
    ...
    private transient long startedCappingAt = -1;
    private transient Player capper;

    public KoTH(String name) {
        this.name = name;
        this.displayName = name;
        this.config = new Config(pCore.getInstance(), this.name, pCore.getInstance().getEventHandler().getKothsFolder());
    ...
    public void end() {
        ...
        Bukkit.broadcastMessage(CC.translate(pCore.getInstance().getEventHandler().getEventsConfig().getString("koth.ended"),
                "%koth%", this.name
        ));
    }
    ...
    public void start() {
        ...
        Bukkit.broadcastMessage(CC.translate(pCore.getInstance().getEventHandler().getEventsConfig().getString("koth.started"),
                "%koth%", this.name
        ));
        runTaskTimerAsynchronously(pCore.getInstance(), 10, 10);
    }

    @Override
    public void win() {
        setActive(false);
        Bukkit.broadcastMessage(CC.translate(pCore.getInstance().getEventHandler().getEventsConfig().getString("koth.captured"),
                "%koth%", this.name,
    ...
    public String getTimeLeftString() {
        return TimeUtils.formatIntoHHMMSS((int) (getTimeLeft() / 1000));
    }

    public long getTimeLeft() {
        if (this.startedCappingAt == -1) return this.defaultCapSeconds;

        return (this.startedCappingAt + this.defaultCapSeconds) - System.currentTimeMillis();
    }
    ...
    public void save() {
        if (this.config.contains("name") && !this.config.getString("name").equals(this.name)) {
            ...
            this.config = new Config(pCore.getInstance(), this.name + ".yml", pCore.getInstance().getEventHandler().getKothsFolder());
            this.config.set("name", this.name);
        }
    ...
    public void delete() {
        ...
        pCore.getInstance().getEventHandler().getEvents().remove(this.name);
    }
