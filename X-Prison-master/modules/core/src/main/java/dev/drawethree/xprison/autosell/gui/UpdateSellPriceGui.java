...
public final class UpdateSellPriceGui extends Gui {
	...
	public UpdateSellPriceGui(Player player, CompMaterial material) {
		super(player, 5, "Editing Sell Price");
		this.material = material;
		this.price = XPrisonAutoSell.getInstance().getManager().getSellPriceFor(material);
	}
	...
	private void setPreviewItem() {
		this.setItem(4, ItemStackBuilder.of(this.material.toItem()).name("&eSell Price").lore(" ", "&7Selling price for this block", String.format("&7is &2$&a%,.2f", this.price)).buildItem().build());
	}
	...
	private void setSaveItem() {
		this.setItem(40, ItemStackBuilder.of(CompMaterial.GREEN_WOOL.toItem()).name("&aSave").lore("&7Click to save the current price.").build(() -> {
			this.saveChanges();
			this.close();
			new SellRegionGui(this.getPlayer()).open();
		}));
	}
	...
	private void saveChanges() {
		XPrisonAutoSell.getInstance().getAutoSellConfig().saveNewItem(this.material, this.price);
	}
