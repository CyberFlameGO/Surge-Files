...
public class EnchantsListener {
	...
	public void register() {
		...
		this.subscribeToItemBreakEvent();
	}
	...
	private void subscribeToItemBreakEvent() {
		Events.subscribe(PlayerItemDamageEvent.class, EventPriority.HIGHEST)
				.filter(e -> !e.isCancelled())
				.filter(e -> e.getPlayer().getItemInHand() != null && this.plugin.getCore().isPickaxeSupported(e.getPlayer().getItemInHand().getType()))
				.handler(e -> e.setCancelled(true)).bindWith(this.plugin.getCore());
	}
