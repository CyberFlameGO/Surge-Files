...
public class GemsManager {
	...
	private final List<UUID> gemsMessageOnPlayers, toggleLightning, toggleNuke, toggleMeteor;
	...
	public GemsManager(XPrisonGems plugin) {
		...
		this.toggleLightning = new ArrayList<>();
		this.toggleNuke = new ArrayList<>();
		this.toggleMeteor = new ArrayList<>();
		this.reload();
	...
	public void toggleLightning(Player p) {
		if (this.toggleLightning.contains(p.getUniqueId())) {
			this.toggleLightning.remove(p.getUniqueId());
		} else {
			this.toggleLightning.add(p.getUniqueId());
		}
	}

	public void toggleNuke(Player p) {
		if (this.toggleNuke.contains(p.getUniqueId())) {
			this.toggleNuke.remove(p.getUniqueId());
		} else {
			this.toggleNuke.add(p.getUniqueId());
		}
	}

	public void toggleMeteor(Player p) {
		if (this.toggleMeteor.contains(p.getUniqueId())) {
			this.toggleMeteor.remove(p.getUniqueId());
		} else {
			this.toggleMeteor.add(p.getUniqueId());
		}
	}
	...
	public boolean hasOffNuke(Player p) { return !this.toggleNuke.contains(p.getUniqueId()); }
	public boolean hasOffMeteor(Player p) { return !this.toggleMeteor.contains(p.getUniqueId()); }
	public boolean hasOffLightning(Player p) { return !this.toggleLightning.contains(p.getUniqueId()); }
