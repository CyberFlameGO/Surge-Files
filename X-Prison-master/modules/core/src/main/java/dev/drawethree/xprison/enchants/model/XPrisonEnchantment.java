...
public abstract class XPrisonEnchantment implements Refundable {
	...
	private boolean enabled, gemEnchant;
	private int guiSlot;
	...
	public void reloadDefaultAttributes() {
		...
		this.gemEnchant = this.plugin.getEnchantsConfig().getYamlConfig().getBoolean("enchants." + id + ".Gem-Enchant", false);
		...
		if (!this.plugin.getEnchantsConfig().getYamlConfig().contains("enchants." + this.id + ".Gem-Enchant")) {
			this.plugin.getEnchantsConfig().getConfig().set("enchants." + this.id + ".Gem-Enchant", false);
			this.plugin.getEnchantsConfig().getConfig().save();
		}
	}
