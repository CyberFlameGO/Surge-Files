...
public final class BlessingEnchant extends XPrisonEnchantment {
    ...
    public void onBlockBreak(BlockBreakEvent e, int enchantLevel) {
        ...
        for (Player p : Players.all()) {
            if (p.getUniqueId() == mine.getOwner()) {
                plugin.getCore().getTokens().getTokensManager().giveTokens(p, amount, null, ReceiveCause.MINING_OTHERS);
            } else {
                long randTaxed = (long) mine.getTaxed((int) amount);
                long diff = amount - randTaxed;

                mine.setTokensTaxed(mine.getTokensTaxed() + diff);
                plugin.getCore().getTokens().getTokensManager().giveTokens(p, randTaxed, null, ReceiveCause.MINING_OTHERS);
            }
