...
public class AutoSellConfig {
    ...
    public void saveNewItem(CompMaterial material, double price) {
        this.getConfig().set("sell-prices.items." + material.name(), price);
        this.getConfig().save();
        XPrisonAutoSell.getInstance().getManager().getSellPrices().put(material, price);
    }

    public void resetItem(CompMaterial material) {
        this.getConfig().set("sell-prices.items." + material.name(), 0.0);
        this.getConfig().save();
        XPrisonAutoSell.getInstance().getManager().getSellPrices().remove(material);
    }
