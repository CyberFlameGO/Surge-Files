...
import dev.drawethree.xprison.enchants.model.impl.gem.*;
...
public class EnchantsRepository {
	...
	public void loadDefaultEnchantments() {
		register(new DoubleOrNothing(this.plugin));
		register(new JackPot(this.plugin));
		register(new LightningBolt(this.plugin));
		register(new MeteorAttack(this.plugin));
		register(new MinionFinder(this.plugin));
