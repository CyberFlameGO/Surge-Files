...
public final class GemEnchantGUI extends Gui {
	...
	public GemEnchantGUI(XPrisonEnchants plugin, Player player, ItemStack pickAxe, int pickaxePlayerInventorySlot) {
	...
	private Item getGuiItem(XPrisonEnchantment enchantment, GemEnchantGUI gui, int currentLevel) {
		...
		return builder.buildItem().bind(handler -> {
			...
			if (handler.getClick() == ClickType.MIDDLE || handler.getClick() == ClickType.SHIFT_RIGHT) {
				this.plugin.getEnchantsManager().buyGemEnchant(enchantment, gui, currentLevel, 100);
				gui.redraw();
			} else if (handler.getClick() == ClickType.LEFT) {
				this.plugin.getEnchantsManager().buyGemEnchant(enchantment, gui, currentLevel, 1);
				gui.redraw();
			} else if (handler.getClick() == ClickType.RIGHT) {
				this.plugin.getEnchantsManager().buyGemEnchant(enchantment, gui, currentLevel, 10);
				gui.redraw();
			} else if (handler.getClick() == ClickType.DROP) {
				this.plugin.getEnchantsManager().buyMaxGemEnchant(enchantment, gui, currentLevel);
			}
	...
	public static void init() {
		...
		GUI_ITEM_LORE = XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getStringList("gem_enchant_menu.item.lore");
		GUI_TITLE = TextUtils.applyColor(XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getString("gem_enchant_menu.title"));
		EMPTY_SLOT_ITEM = ItemStackBuilder.
				of(CompMaterial.fromString(XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getString("gem_enchant_menu.empty_slots")).toItem()).buildItem().build();
		GUI_LINES = XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getInt("gem_enchant_menu.lines");

		HELP_ITEM_ENABLED = XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getBoolean("gem_enchant_menu.help_item.enabled", true);
		PICKAXE_ITEM_ENABLED = XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getBoolean("gem_enchant_menu.pickaxe_enabled", true);

		if (HELP_ITEM_ENABLED) {
			String base64 = XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getString("gem_enchant_menu.help_item.Base64", null);

			if (base64 != null) {
				HELP_ITEM = ItemStackBuilder.of(SkullUtils.getCustomTextureHead(base64))
						.name(XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getString("gem_enchant_menu.help_item.name")).lore(XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getStringList("gem_enchant_menu.help_item.lore")).buildItem().build();
			} else {
				HELP_ITEM = ItemStackBuilder.of(CompMaterial.fromString(XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getString("gem_enchant_menu.help_item.material")).toMaterial())
						.name(XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getString("gem_enchant_menu.help_item.name")).lore(XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getStringList("gem_enchant_menu.help_item.lore")).buildItem().build();
			}

			HELP_ITEM_SLOT = XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getInt("gem_enchant_menu.help_item.slot");

		}

		if (PICKAXE_ITEM_ENABLED) {
			PICKAXE_ITEM_SLOT = XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getInt("gem_enchant_menu.pickaxe_slot");
		}
	}
}
