...
public class XPrisonMVdWPlaceholder {
	...
	private void registerMinesPlaceholders() {
		...
		if (!this.plugin.isPCoreEnabled()) {
			return;
		}

		PlaceholderAPI.registerPlaceholder(plugin, "xprison_mine_blocks_left", event -> {
			PrivateMine mine = pCore.getInstance().getPrivateMineHandler().fetchCache(event.getPlayer().getUniqueId());

			if (mine == null) return "";

			return String.format("%,d", mine.getBlocksLeft());
		});
		PlaceholderAPI.registerPlaceholder(plugin, "xprison_mine_progress", event -> {
			PrivateMine mine = pCore.getInstance().getPrivateMineHandler().fetchCache(event.getPlayer().getUniqueId());

			if (mine == null) return "";

			return String.format("%,.2f", (double) mine.getBlocksLeft() / mine.getTotalBlocks() * 100.0D);
		});
		PlaceholderAPI.registerPlaceholder(plugin, "xprison_mine_reset_time", event -> {
			PrivateMine mine = pCore.getInstance().getPrivateMineHandler().fetchCache(event.getPlayer().getUniqueId());

			if (mine == null) return "";

			return TimeUtil.getTime(mine.getNextResetMillis() / 1000);
		});
	}
