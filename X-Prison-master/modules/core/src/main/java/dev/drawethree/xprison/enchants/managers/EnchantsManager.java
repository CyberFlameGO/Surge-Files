...
import dev.drawethree.xprison.enchants.gui.GemEnchantGUI;
...
public class EnchantsManager {
	...
	public void handleBlockBreak(BlockBreakEvent e, ItemStack pickAxe) {
		...
		if (mine == null) {
			this.plugin.getCore().debug("EnchantsManager::handleBlockBreak >> No mine found. Enchants will not be triggered.", this.plugin);
			return;
		}
	...
	public void buyGemEnchant(XPrisonEnchantment enchantment, GemEnchantGUI gui, int currentLevel, int addition) {
		...
		if (addition == 1) {
			PlayerUtils.sendMessage(gui.getPlayer(), plugin.getEnchantsConfig().getMessage("enchant_bought_gems").replace("%gems%", String.format("%,d", totalCost)));
		} else {
			PlayerUtils.sendMessage(gui.getPlayer(), plugin.getEnchantsConfig().getMessage("enchant_bought_multiple_gems")
					.replace("%amount%", String.valueOf(addition))
					.replace("%enchant%", enchantment.getName())
					.replace("%gems%", String.format("%,d", totalCost)));
		}
	}
	...
	public void disenchant(XPrisonEnchantment enchantment, DisenchantGUI gui, int currentLevel, int substraction) {
		...
		if (enchantment.isGemEnchant()) {
			PlayerUtils.sendMessage(gui.getPlayer(), plugin.getEnchantsConfig().getMessage("enchant_gems_back").replace("%gems%", String.format("%,d", totalRefunded)));
		} else {
	...
	public void buyMaxGemEnchant(XPrisonEnchantment enchantment, GemEnchantGUI gui, int currentLevel) {
