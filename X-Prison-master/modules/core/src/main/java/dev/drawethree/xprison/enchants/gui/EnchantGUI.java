...
public final class EnchantGUI extends Gui {
	...
	public void redraw() {
		...
		if (GEM_ITEM_ENABLED) {
			this.setItem(GEM_ITEM_SLOT, ItemStackBuilder.of(GEM_ITEM).build(() -> {
				this.close();
				new GemEnchantGUI(this.plugin, this.getPlayer(), this.pickAxe, this.pickaxePlayerInventorySlot).open();
			}));
		}
	...
	public static void init() {
		...
		GEM_ITEM_ENABLED = XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getBoolean("enchant_menu.gem_item.enabled", true);
		...
		if (GEM_ITEM_ENABLED) {
			String base64 = XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getString("enchant_menu.gem_item.Base64", null);

			if (base64 != null) {
				GEM_ITEM = ItemStackBuilder.of(SkullUtils.getCustomTextureHead(base64))
						.name(XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getString("enchant_menu.gem_item.name")).lore(XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getStringList("enchant_menu.gem_item.lore")).build();
			} else {
				GEM_ITEM = ItemStackBuilder.of(CompMaterial.fromString(XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getString("enchant_menu.gem_item.material")).toMaterial())
						.name(XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getString("enchant_menu.gem_item.name")).lore(XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getStringList("enchant_menu.gem_item.lore")).build();
			}
			GEM_ITEM_SLOT = XPrisonEnchants.getInstance().getEnchantsConfig().getYamlConfig().getInt("enchant_menu.gem_item.slot");
