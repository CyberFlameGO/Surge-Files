...
public final class ExplosiveEnchant extends XPrisonEnchantment {
    ...
    public void onBlockBreak(BlockBreakEvent e, int enchantLevel) {
        ...
        this.plugin.getCore().debug("ExplosiveEnchant::onBlockBreak >> WG Region used: " + mine.getOwner(), this.plugin);
        int radius = this.calculateRadius(enchantLevel);

        List<Block> blocksAffected = this.blockProvider.provide(b, radius).stream().filter(block -> mine.getMinePitBox().contains(block.getLocation()) && block.getType() != Material.AIR).collect(Collectors.toList());

        ExplosionTriggerEvent event = this.callExplosionTriggerEvent(e.getPlayer(), mine, e.getBlock(), blocksAffected);
    ...
    private ExplosionTriggerEvent callExplosionTriggerEvent(Player p, PrivateMine mine, Block originBlock, List<Block> blocks) {
        ExplosionTriggerEvent event = new ExplosionTriggerEvent(p, mine, originBlock, blocks);
