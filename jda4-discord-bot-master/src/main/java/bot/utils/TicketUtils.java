...
public class TicketUtils {
    ...
    public static void closeTicket(String reason, Guild guild, User closed, TextChannel tc) {
        ...
        sb.append("# MineSurge Ticket Logs");
        sb.append("\n");
    ...
    public static void handleNewTicket(Guild guild, Member member, String reason, String type)
            throws InterruptedException, ExecutionException {

        Ticket config = DataSource.INS.getTicketConfig(guild.getId());
        String roleId = "1116046249817292830";
        if (config == null)  {
            System.out.println("stopped here 3");
            return;
        }

        // Cyrillic "і" is used
        ChannelAction<TextChannel> action = guild.createTextChannel("tіcket-" + member.getEffectiveName(), guild.getCategoryById("994327524744101968"))
                .setTopic("tіcket" + "|" + member.getId() + "|" + type)
                .addPermissionOverride(member, Arrays.asList(Permission.MESSAGE_READ, Permission.MESSAGE_WRITE, Permission.MESSAGE_ATTACH_FILES),
                ...
                .addPermissionOverride(Objects.requireNonNull(guild.getMember(guild.getSelfMember().getUser())),
                        Arrays.asList(Permission.MESSAGE_READ, Permission.MESSAGE_WRITE, Permission.MESSAGE_ATTACH_FILES), Collections.emptyList());
        ...
        Message msg = tc.sendMessage(meb.build())
                .setActionRow(Button.danger("close-ticket", Constants.LOCK + " Close"),
                        Button.primary("ticket-elevate", "▲ Elevate Ticket"),
                        Button.primary("ticket-claim", "♛ Claim Ticket"))
                .submit().get();

        if (roleId != null) {
            tc.sendMessage(member.getAsMention() + " " + guild.getRoleById(roleId).getAsMention()).queue();
        } else {
        ...
        EmbedBuilder eb = EmbedUtils.getDefaultEmbed()
                ...
                .setDescription(""
                        + Constants.WHITE_SMALL_SQUARE + "**Ticket:** " + member.getEffectiveName() + "\n"
