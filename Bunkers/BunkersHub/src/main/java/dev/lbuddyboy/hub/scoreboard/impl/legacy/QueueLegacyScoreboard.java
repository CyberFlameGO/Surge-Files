package dev.lbuddyboy.hub.scoreboard.impl.legacy;
...
public class QueueLegacyScoreboard extends HubScoreboard {

    private final Config config;
    private final ScoreboardTitle title;
    private final List<ScoreboardLine> lines = new ArrayList<>();

    public QueueLegacyScoreboard() {
        this.config = new Config(lHub.getInstance(), "legacy-queue", lHub.getInstance().getScoreboardHandler().getScoreboardDirectory());
    ...
    public List<String> translateLines(List<String> lines, Player player) {
        ...
        for (String line : lines) {
            ...
            replacement.add(line
                    .replaceAll("%queue%", queueImpl.getQueueName(player))
                    .replaceAll("%pos%", String.valueOf(queueImpl.getPosition(player)))
                    .replaceAll("%size%", String.valueOf(queueImpl.getQueueSize(player)))
                    ...
                    .replaceAll("%date%", StringUtils.center(sdf.format(new Date()), CC.translate(getTitle().getActiveFrame()).length()))
            );
        }
    ...
    public boolean qualifies(Player player) {
        ...
        return Via.getAPI().getPlayerVersion(player.getUniqueId()) <= 340 && queueImpl.inQueue(player);
    }
}
